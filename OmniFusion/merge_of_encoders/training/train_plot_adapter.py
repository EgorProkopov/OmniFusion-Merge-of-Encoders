...
from transformers import Adafactor, get_cosine_schedule_with_warmup, AutoTokenizer, AutoModelForCausalLM

from deplot import DePlotVisionTower
from model.src.charts_dataset import get_dataset, get_collate_function
from model.src.model.models import CLIPVisionTower, VisualToGPTMapping
...
class Model_pl(pl.LightningModule):
    def __init__(
        ...
        self.loss_fct = torch.nn.CrossEntropyLoss(reduction="none", ignore_index=cfg.pad_id)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx % 25000 == 0:
            ...
            torch.save(self.encoder_projection,
                       f"ckpts/{self.cfg.exp_name}/{batch_idx}/{self.encoder_name}_projection.pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default='D:\\airi\\Chart2Text\\model\\src\\DePlot\\config-deplot-adapter-pretrain.json')
    ...
    encoder = DePlotVisionTower("google/deplot")  # TODO: codetr, plots, ocr, etc.
    encoder.load_model()
